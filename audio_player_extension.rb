...
class AudioPlayerExtension < Radiant::Extension
  ...
  define_routes do |map|
    ...
    map.namespace :admin, :member => { :remove => :get } do |admin|
      admin.resources :audio,                :controller => "audio", :singular => 'audio_file', :collection => { :reorder => :get, :update_order => :post }
      admin.resources :audio_player_configs, :controller => "audio_player_config"
    end
